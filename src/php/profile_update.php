...
require_once('initialize.php');
    ...
    $query = mysqli_query($con,"SELECT email FROM `users` WHERE email= '$email' AND userID != $userID ");
        ...
        if(mysqli_num_rows($query)>0){
            header("location: ../../public/admin/admin_profile.php?error=Email already exists.");
            exit();
        }

        if($password != $confirm_password){
            header("location: ../../public/admin/admin_profile.php?error=Confirm password not matching.");
            exit();
       }
       ...
       $password = password_hash($password, PASSWORD_DEFAULT);
        ...
        if($password != '') {     
            ...
            header("location: ../../public/admin/admin_profile.php");
        }else {
